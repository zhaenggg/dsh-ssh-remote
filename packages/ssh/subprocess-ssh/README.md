...
Service Provider for the [`@deepseek-ai/dsh-subprocess`](../../subprocess/subprocess/README.md) capability on a remote SSH host: `SshSubprocessRuntime` (default export, `ctx.subprocess`) executes each spawn spec as one `ssh2` exec channel on the shared [`ctx.ssh`](../ssh/README.md) connection. [`@deepseek-ai/dsh-fs-routing`](../fs-routing/README.md) constructs one instance per host key; the spec's `cwd` selects the host.
