...
# The published files lists expect built lib/ output — build each package first
# (tsdown, inside a deepseek-harness workspace checkout where the @deepseek-ai
# dev dependencies resolve), or publish from a prepared tree with lib/ present.
set -euo pipefail
cd "$(dirname "$0")/.."
